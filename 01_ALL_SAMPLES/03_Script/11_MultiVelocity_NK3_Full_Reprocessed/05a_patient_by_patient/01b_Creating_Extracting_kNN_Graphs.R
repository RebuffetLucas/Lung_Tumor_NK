...
#Performing the pre-processing on patient samples one by one, only on cells that will be kept for MultiVelocity Analysis
...
seurat_object <- RunPCA(seurat_object, npcs= DIM_PCA,verbose = FALSE)
seurat_object <- RunUMAP(seurat_object, dims = 1:DIM_PCA, reduction.name = 'umap.rna', reduction.key = 'rnaUMAP_') # optional
...
seurat_object <- RunSVD(seurat_object, n=DIM_LSI)
seurat_object <- RunUMAP(seurat_object, reduction = 'lsi', dims = 2:DIM_LSI, reduction.name = "umap.atac", reduction.key = "atacUMAP_") # optional

# find weighted nearest neighbors
seurat_object <- FindMultiModalNeighbors(seurat_object, reduction.list = list("pca", "lsi"), dims.list = list(1:DIM_PCA, 2:DIM_LSI), knn.range= 40) 
