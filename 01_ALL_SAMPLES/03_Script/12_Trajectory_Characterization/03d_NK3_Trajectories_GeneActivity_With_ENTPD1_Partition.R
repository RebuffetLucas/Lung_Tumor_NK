...
RNA_OR_GENEACTIV = "GeneActivity" # "GeneActivity"

#Load the saved slots and recreate a Seurat Object from scratch

counts = readRDS(file.path(PATH_EXPERIMENT_OUTPUT, "10_DiffusionMap", "Object_GeneActivty_Before_RUNFast_MNN",  "count.data.gene_activity_all.rds")) # Using all the RNA
data = readRDS( file.path(PATH_EXPERIMENT_OUTPUT, "10_DiffusionMap", "Object_GeneActivty_Before_RUNFast_MNN",  "log.norm.data.gene_activity_all.rds"))
metadata = readRDS(file.path(PATH_EXPERIMENT_OUTPUT, "10_DiffusionMap", "Object_GeneActivty_Before_RUNFast_MNN",  "metadata.rds"))
var.features = readRDS(file.path(PATH_EXPERIMENT_OUTPUT, "10_DiffusionMap" , "Object_GeneActivty_Before_RUNFast_MNN",  "var.features.gene_activity_q95.rds"))
...
object_for_reduction= readRDS(file.path(PATH_EXPERIMENT_OUTPUT, "10_DiffusionMap", "Object_GeneActivty_Before_RUNFast_MNN",  "object_GeneActivity.rds"))
...
#NK_Seurat = readRDS(file.path(PATH_EXPERIMENT_OUTPUT, "10_DiffusionMap", "Object_GeneActivty_Before_RUNFast_MNN",  "object.rds"))
...
FILE = file.path(PATH_ANALYSIS_OUTPUT , RNA_OR_GENEACTIV , "Trajectories_plots", TRAJ)
...
#FILE = file.path(PATH_ANALYSIS_OUTPUT , RNA_OR_GENEACTIV , "modulated_genes", TRAJ)
#dir.create(FILE, showWarnings = FALSE)
...
pdf(file= FILE, width = 3.6, height = 10 , paper = "a4")
draw(ht_list)
dev.off()
...
pdf(file= FILE, width = 2.9, height = 8 , paper = "a4")
draw(ht_list2)
dev.off()
...
pdf(file= FILE, width = 4.3, height = 8 , paper = "a4")
draw(ht_list2)
dev.off()
